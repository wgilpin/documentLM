(() => {
  ...
  function run$1(config2) {
    var _a2;
    const { editor: editor2, from: from2, to, text: text2, rules, plugin } = config2;
    const { view } = editor2;
    if (view.composing) {
      return false;
    }
        ...
        new Plugin({
          ...
          props: {
            clipboardTextSerializer: () => {
              const { editor: editor2 } = this;
              const { state, schema: schema2 } = editor2;
        ...
        toggleTight: function() {
          ...
          return (_ref) => {
            ...
            function toggleTight(name) {
              if (!editor2.isActive(name)) {
                return false;
              }
              const attrs2 = editor2.getAttributes(name);
...
`;
  ...
  // static/editor-utils.js
  function findBlockInMarkdown(lines, nodeText, minOffset = 0) {
    let offset = 0;
    for (const line of lines) {
      if (offset >= minOffset) {
        const isHeading = /^#{1,6}\s+/.test(line);
        let plain = line.replace(/^#{1,6}\s+/, "").replace(/^[-*+]\s+/, "");
        if (!isHeading) plain = plain.replace(/^\d+\.\s+/, "");
        plain = plain.replace(/^>\s*/, "").replace(/\*\*(.+?)\*\*/g, "$1").replace(/\*(.+?)\*/g, "$1").replace(/`(.+?)`/g, "$1").replace(/~~(.+?)~~/g, "$1").replace(/\\(.)/g, "$1").trim();
        if (plain === nodeText) {
  ...
  // static/suggestion-utils.js
  function stripMarkdownBasic(text2) {
    return text2.replace(/^#{1,6}\s+/gm, "").replace(/\*\*\*(.+?)\*\*\*/gs, "$1").replace(/\*\*(.+?)\*\*/gs, "$1").replace(/\*(.+?)\*/gs, "$1").replace(/`(.+?)`/gs, "$1").replace(/~~(.+?)~~/gs, "$1").replace(/\[(.+?)\]\(.+?\)/g, "$1").replace(/\\(.)/g, "$1").trim();
  }
  function findTextInDoc(doc3, searchText) {
    const plainText = stripMarkdownBasic(searchText);
    if (!plainText) return null;
    let text2 = "";
    const posMap = [];
    doc3.nodesBetween(0, doc3.content.size, (node, pos) => {
      if (node.isText) {
    ...
    const idx = text2.indexOf(plainText);
  ...
  function adaptSuggestionForListItem(md2, suggestedText, mdRange) {
    const originalLine = md2.slice(mdRange.start, mdRange.end);
    ...
    return lines.map((line, i) => i === 0 || line === "" ? line : indent + line).join("\n");
  }
  function applySuggestionToMarkdown(md2, suggestedText, mdRange) {
    const adaptedText = adaptSuggestionForListItem(md2, suggestedText, mdRange);
    return md2.slice(0, mdRange.start) + adaptedText + md2.slice(mdRange.end);
  }

  // static/editor.js
  var mountEl = document.getElementById("tiptap-mount");
  var contentInput = document.getElementById("document-content");
  var aiBtn = document.getElementById("ai-block-btn");
  var pendingSelection = { start: 0, end: 0, text: "" };
  var lastChangeIsAi = false;
  var docIdMatch = contentInput.getAttribute("hx-put")?.match(/\/documents\/(.+)$/);
  var DOC_ID = docIdMatch ? docIdMatch[1] : "unknown-doc";
  var HISTORY_KEY = `tiptap-history-${DOC_ID}`;
  var localHistory = [];
  var localHistoryIndex = -1;
  try {
  ...
  function saveToHistory(text2) {
    if (localHistory[localHistoryIndex] === text2) return;
    if (localHistoryIndex < localHistory.length - 1) {
      localHistory = localHistory.slice(0, localHistoryIndex + 1);
    }
    localHistory.push(text2);
  ...
  function canUndoLocal() {
    return localHistoryIndex > 0;
  }
  function canRedoLocal() {
    return localHistoryIndex < localHistory.length - 1;
  }
  var activeBlockPos = null;
  var activeBlockNode = null;
  function updateAiButton(editor2) {
  ...
  function updateToolbar(editor2) {
    const canUndo = canUndoLocal();
    const canRedo = canRedoLocal();
    undoBtn.disabled = !canUndo;
    redoBtn.disabled = !canRedo;
    undoBtn.title = canUndo ? lastChangeIsAi ? "Undo AI change" : "Undo" : "Nothing to undo";
  ...
  var pendingAiPmRange = null;
  var pendingAiMdRange = null;
  var suggestionRanges = /* @__PURE__ */ new Map();
  function buildSuggestionDecorations(doc3) {
    const decorations = [];
    for (const s of pendingSuggestions) {
      const stored = suggestionRanges.get(s.id);
      const range = stored?.pm ?? findTextInDoc(doc3, s.original_text);
      if (!range) continue;
      decorations.push(
  ...
  var suggestionPlugin = new Plugin({
    key: suggestionsPluginKey,
    state: {
      init(_, { doc: doc3 }) {
        return buildSuggestionDecorations(doc3);
      },
      apply(tr, old) {
  ...
  var SuggestionDecorations = Extension.create({
  ...
  window.acceptSuggestion = async function(btn) {
    ...
    const stored = suggestionRanges.get(s.id);
    if (stored?.md) {
      const md2 = window.tiptapEditor.storage.markdown.getMarkdown();
      const newMd = applySuggestionToMarkdown(md2, s.suggested_text, stored.md);
      ...
      if (range) {
        window.tiptapEditor.chain().deleteRange(range).insertContentAt(range.from, stripMarkdownBasic(s.suggested_text)).run();
      }
    }
  ...
  aiBtn.addEventListener("click", (e) => {
    ...
    let blockPositions = [];
    console.log("[AI btn] click \u2014 empty:", empty2, "activeBlockPos:", activeBlockPos, "node:", activeBlockNode?.textContent?.slice(0, 40));
    if (empty2) {
      if (activeBlockPos === null || !activeBlockNode) {
        console.warn("[AI btn] no active block \u2014 pos:", activeBlockPos, "node:", activeBlockNode);
        return;
      }
      const nodeText = activeBlockNode.textContent.trim();
      const found2 = findBlockInMarkdown(lines, nodeText);
      console.log("[AI btn] nodeText:", nodeText.slice(0, 60), "| found:", !!found2);
      if (!found2) return;
      selStart = found2.start;
      selEnd = found2.end;
      selText = md2.slice(selStart, selEnd);
      ...
      if (blocks.length === 0) {
        console.warn("[AI btn] no blocks in selection");
        return;
      }
      const first2 = findBlockInMarkdown(lines, blocks[0].text);
      if (!first2) {
        console.warn("[AI btn] first block not found in markdown:", blocks[0].text.slice(0, 60));
        return;
      }
      let last = first2;
      let searchFrom = first2.end;
      for (let i = 1; i < blocks.length; i++) {
        const found2 = findBlockInMarkdown(lines, blocks[i].text, searchFrom);
        if (!found2) {
          console.warn("[AI btn] block not found:", blocks[i].text.slice(0, 60));
          return;
        }
        last = found2;
        searchFrom = found2.end;
      }
      selStart = first2.start;
      selEnd = last.end;
      selText = md2.slice(selStart, selEnd);
      blockPositions = blocks.map((b) => b.pos);
    }
    if (!selText) {
      console.warn("[AI btn] empty selText");
      return;
    }
    if (empty2) {
      pendingAiPmRange = {
      ...
      pendingAiPmRange = { from: from2, to };
    }
  ...
  var editorPane = mountEl.closest(".editor-pane");
  new MutationObserver((mutations) => {
  ...
  var toolbarContainer = document.querySelector(".tiptap-editor");
  if (toolbarContainer) {
